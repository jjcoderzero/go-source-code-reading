...
// 我们有两种处理延迟的方法。旧的方法包括在执行defer语句时创建一个defer记录，并将其添加到一个defer链中。这个链由deferreturn调用在所有函数出口上检查，以便运行适当的defer调用。
// 对于循环中不出现defer语句的函数，可以使用一种更方便的方法(我们称之为开放编码延迟方法)。在这种情况下，我们只是将defer function/arg信息存储到每个defer语句的特定堆栈槽中，并在位掩码中设置一个位。
// 在每个函数退出时，我们添加内联代码，以便根据存储在堆栈上的位掩码和fn/arg信息直接进行适当的defer调用。在panic/Goexit处理期间，适当的defer调用是使用额外的funcdata信息来表示包含位掩码和延迟fn/args的确切堆栈插槽。

// 确保我们真的能引起panic。如果panic是从runtime或malloc内部生成的，则转换为一次msg抛出。pc应该是程序计数器的编译器产生的代码，触发了这种panic。
func panicCheck1(pc uintptr, msg string) {
	if sys.GoarchWasm == 0 && hasPrefix(funcname(findfunc(pc)), "runtime.") {
		// Note: wasm不能跟踪调用，所以我们不能得到原始调用者的pc。
		throw(msg)
	}
...
// 与上面相同，但是允许从runtime调用。使用此函数对于runtime.sigpanic可能生成的任何panic都是必要的，因为它们总是由runtime调用
func panicCheck2(err string) {
	// panic allocates, so to avoid recursive malloc, turn panics
	// during malloc into throws.
	// panic分配，因此为了避免递归的malloc，将malloc期间的panics转化为throws。
	gp := getg()
	if gp != nil && gp.m != nil && gp.m.mallocing != 0 {
		throw(err)
	}
...
// 创建一个新的带有siz字节参数的延迟函数fn。编译器将一个defer语句转换为对它的调用。
//go:nosplit
func deferproc(siz int32, fn *funcval) { // arguments of fn follow fn
	...
	// fn的争论处于危险状态。延迟程序的堆栈映射没有描述它们。所以我们不能让垃圾收集或堆栈复制触发，直到我们把它们复制到安全的地方。下面的memmove就可以做到这一点。在复制完成之前，我们只能调用nosplit例程。
	sp := getcallersp()
